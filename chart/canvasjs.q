...
.st.options: raze (.st.d[`zoomEnabled; 1b]; .st.d[`axisX; .st.d[`labelAngle; -30]]; .st.d[`axisY; .st.d[`includeZero; 0b]]);
.st.columnDataSeries: {.st.d[`data] {.st.d[`type; `column], .st.d[`showInLegend; 1b], .st.d[`legendText; cols[x]1], .st.d[`dataPoints; `label`y xcol x]} each x};
.st.lineDataSeries: {.st.options, .st.d[`data] {.st.d[`type; `line], .st.d[`markerType; `none], .st.d[`showInLegend; 1b], .st.d[`legendText; cols[x]1], .st.d[`dataPoints; `x`y xcol x]} each x};
.st.pointDataSeries: {.st.options, .st.d[`data] {.st.d[`type; `scatter], .st.d[`showInLegend; 1b], .st.d[`legendText; cols[x]1], .st.d[`dataPoints; `x`y xcol x]} each x};
.st.bubbleDataSeries: {.st.d[`data] {.st.d[`type; `bubble], .st.d[`showInLegend; 1b], .st.d[`legendText; cols[x]1], .st.d[`dataPoints; `x`y`z xcol x]} each x};
...
.st.optionalLegendFromSym: {$[`sym in cols x; .st.d[`showInLegend; 1b], .st.d[`legendText; first x`sym]; .st.d[`showInLegend; 0b], .st.d[`legendText; `]]};
.st.volumnDataSeries: {.st.d[`type; `column], .st.d[`showInLegend; 0b], .st.d[`legendText; `], .st.d[`dataPoints; `x`y xcol x]}';
